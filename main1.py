...
def detect_intent(user_text: str):
    ...
    if any(w in text for w in ["insurance", "covered", "aetna", "bluecross"]):
        return "insurance"
...
# Prompt Builder
# ----------------------------------
def build_prompt(session_id: str, user_text: str, system_info: str):
    history = ""
    for u, a in sessions[session_id]["history"]:
        history += f"User: {u}\nAssistant: {a}\n"

    return (
        f"You are a polite virtual receptionist for Confido Health Clinic.\n"
        f"Clinic info: {clinic_info}\n"
        f"Insurance info: {insurance_db}\n"
...
async def voice_assistant(file: UploadFile = File(...), session_id: str = Form("default")):
    ...
    if not user_text_clean:
        ...
        if intent == "appointment":
            structured_reply = handle_appointment(session_id, user_text_clean)
            prompt = build_prompt(session_id, user_text_clean, f"The assistant must say: {structured_reply}")
            reply = get_llm_response(prompt)

        elif intent == "insurance":
            structured_reply = handle_insurance(user_text_clean)
            prompt = build_prompt(session_id, user_text_clean, f"The assistant must explain politely: {structured_reply}")
            reply = get_llm_response(prompt)

        elif intent == "faq":
            structured_reply = handle_faq(user_text_clean)
            prompt = build_prompt(session_id, user_text_clean, f"The assistant must answer this FAQ: {structured_reply}")
            reply = get_llm_response(prompt)

        else:  # chit-chat or fallback
            prompt = build_prompt(session_id, user_text_clean, "Respond naturally and conversationally.")
            reply = get_llm_response(prompt)
